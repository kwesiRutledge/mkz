function [varargout] = get_mpc_matrices(varargin)
	% get_mpc_matrices
	%	Description:
	%		Creates MPC matrices.
	%		This is modified to allow for missing observations if necessary.
	%
	%	Usage:
	%		[G,H,C_big,x0_mat] = get_mpc_matrices(sys,T)
	%		[G,H,C_big,x0_mat] = get_mpc_matrices(sys,T,'missing',m_locs)
	%		[G,H,C_big,x0_mat,E_bar,G_bar] = get_mpc_matrices(sys,T)
	%
	%	Inputs:
	...
	%Constants
	if nargin == 2 %~exist('m_locs')
		%This temporary condition will make code generation possible (hopefully)
		m_locs = [];
	end
